...
- **Fixtures** — bought items at real sizes, in the **Add a fixture** strip and under **Parts → Pieces**. Outdoor kitchen: drop-in gas grill, grill cart, pellet smoker, kamado, wood-fired and portable pizza ovens, undercounter fridge, bar sink, concrete countertop (cut to length), cabinet doors and drawer fronts (they stick to a face), bar stool, fire pit ring. Clay: adobe block, flue liner, terracotta tile. Beds & play: twin / twin XL / full / toddler mattresses, pillow, foam play mat, bean bag. Lighting & shade: string lights (cut to length, drawn as a swagged cord with bulbs), hanging, wall and table lanterns, solar path light, tiki torch, cantilever umbrella, patio umbrella. In the real view, **Evening** drops the sun low and makes the bulbs, lanterns and torches glow.
...
- **Custom parts** — select a group or a piece in Build (or nothing, for the whole build) and tap **Save as part** to name it and keep it. Saved parts live on the device and show up in every project: as cards in Build's **Custom parts** strip and under **Parts → Custom**, where you can rename or delete them. Adding one places a fresh copy as a group. Assemblies are converted to real pieces when saved. **Share** on a custom part makes a link that carries the part — open it on another device and it lands in that device's library. **Export all** downloads the whole library as `fortkit-parts.json`; **Import file** reads one back (parts already on the device are skipped).
- **Parts** — the full catalog: Pieces (true stock), Assemblies (1 ft-grid parts), and your Custom parts, filterable by material.
